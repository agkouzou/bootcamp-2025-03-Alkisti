    ...
    private User getCurrentUser(Authentication authentication) throws BootcampException {
        try {
            ...
            return userRepository.findById(userId)
                    .orElseThrow(() -> new BootcampException(HttpStatus.UNAUTHORIZED, "User not found with id: " + userId));
        } catch (NumberFormatException e) {
            throw new BootcampException(HttpStatus.UNAUTHORIZED, "Invalid user ID in token: " + e.getMessage(), e);
        } catch (ClassCastException e) {
            throw new BootcampException(HttpStatus.UNAUTHORIZED, "Authentication principal is not a valid JWT token.", e);
        }
    }
